...
def session_restart():
    ...
    try:
        ...
        _SESSION_REVOKED = False
        setup.start_phone(phone)
        record_service_event("session_restart_requested", "تم إلغاء الجلسة السابقة وبدء جلسة جديدة")
        return jsonify({"ok": True, "message": "تم إلغاء الجلسة السابقة وبدأ إرسال كود Telegram للجلسة الجديدة.", "state": "starting"})
    except (OSError, URLError, HTTPError, RuntimeError) as exc:
        return json_error(str(exc))
