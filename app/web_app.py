...
<script>
let state={};
async function api(url, options={}){const r=await fetch(url,{credentials:'same-origin',headers:{'Content-Type':'application/json'},...options});const raw=await r.text();let d=null;try{d=raw?JSON.parse(raw):null}catch(_e){}if(!r.ok){if(d&&d.error)throw new Error(d.error);if(raw.trim().startsWith('<'))throw new Error('خدمة Render تستيقظ أو تعيد النشر الآن؛ انتظر قليلًا ثم أعد المحاولة.');throw new Error(`تعذر تنفيذ الطلب (${r.status})`)}if(!d)throw new Error('الخادم أعاد ردًا غير صالح؛ أعد المحاولة بعد لحظات.');return d}
