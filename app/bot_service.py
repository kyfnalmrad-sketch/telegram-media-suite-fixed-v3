...
class TelegramBotService:
    ...
    def start(self, api_id: str, api_hash: str, bot_token: str,
        ...
        return True, "بدأ تشغيل البوت لمدة أربع ساعات"
    ...
    def _error_report(exc: Exception, operation: str) -> str:
        ...
        if isinstance(exc, ValueError) and ("غير موجود" in str(exc) or "not found" in detail):
        ...
        elif isinstance(exc, ValueError) and ("رابط" in str(exc) or "غير صالح" in str(exc)):
            cause = "صيغة رابط Telegram غير صحيحة أو لا تحتوي رقم رسالة صالحًا."
            fix = "استخدم رابطًا مثل https://t.me/channel/123 أو https://t.me/c/4405004978/3/20، وتأكد من نسخ الرابط كاملًا."
        elif isinstance(exc, ValueError):
            cause = "تعذر الوصول إلى الرسالة أو أن Telegram لم يعثر عليها بهذه الصيغة الداخلية."
            fix = "تأكد أن الحساب الشخصي عضو في القناة الخاصة، وأن الرسالة متاحة، ثم جرّب الرابط مرة أخرى. إذا كان الرابط موضوعًا مثل /3/20 فسيستخدم النظام رقم الرسالة الأخير 20."
        elif isinstance(exc, RuntimeError) and ("عضو" in str(exc) or "مشترك" in str(exc)):
            cause = "الحساب الشخصي غير مشترك في القناة المطلوبة."
            fix = "اشترك بالحساب الشخصي في القناة أولًا، ثم أعد فحص الرابط. لا يستطيع النظام تجاوز اشتراك Telegram أو الانضمام تلقائيًا دون موافقة منك."
            ...
            cause = "الحساب الشخصي لا يستطيع الوصول إلى القناة أو لا يملك الصلاحية المطلوبة."
            fix = "تأكد أن الحساب عضو في القناة الخاصة، أو أرسل رابطًا عامًا صحيحًا، ولا تستخدم رابط دعوة لم يتم الانضمام إليه."
    ...
    async def _continue_flow(self, message: Any, text: str, flow: dict[str, Any]) -> None:
        ...
        if mode in {"info", "download"}:
            links = self._extract_links(text)
            if not links:
                await message.reply_text("السبب: لم أجد رابط Telegram قابلًا للتحليل.\nالإصلاح: أرسل رابطًا مثل t.me/channel/123 أو https://t.me/c/4405004978/3، ويمكن إرسال عدة روابط.", reply_markup=self._reply_keyboard())
                return
            if mode == "download":
                ...
                for link in links:
                    try:
                        ...
                        try:
                            parse_chat_link(link)
                            channel_links.append(link)
                        except ValueError:
                            pass
                if channel_links and not message_links:
                    # رابط القناة لا يملك رقم رسالة؛ مرّره إلى تدفق القناة
                    # كي يحدد النوع ويسأل عن الاشتراك قبل اختيار الوسائط والتنزيل.
                    flow = {"mode": "channel_link", "created_at": time.time()}
                    self.user_flows[user_id] = flow
                    await self._continue_flow(message, channel_links[0], flow)
                    return
            self.user_flows.pop(user_id, None)
            if mode == "info":
        ...
        if mode == "channel_link":
            links = self._extract_links(text)
            if not links:
                await message.reply_text("السبب: لم أجد رابط قناة أو رسالة Telegram قابلًا للتحليل.\nالإصلاح: أرسل t.me/channel أو t.me/channel/123 أو https://t.me/c/4405004978/3، ويمكن أيضًا إرسال رابط دعوة.", reply_markup=self._reply_keyboard())
                return
            text = links[0]
            try:
        ...
        if mode in {"range_link_messages", "range_link_time"}:
            links = self._extract_links(text)
            if not links:
                await message.reply_text("السبب: لم أجد رابط قناة Telegram.\nالإصلاح: أرسل t.me/channel أو https://t.me/c/4405004978/3.", reply_markup=self._reply_keyboard())
                return
            try:
    ...
    async def _download_one(self, message: Any, link: str, worker_id: int = 1) -> bool:
        try:
            ...
            self.log(f"فشل طلب البوت: {type(exc).__name__} — {detail}")
