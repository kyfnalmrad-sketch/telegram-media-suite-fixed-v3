...
async def deliver(bot: Client, message: Message, link: str) -> None:
    ...
    status = await message.reply_text("⏳ جارٍ تنزيل الفيديو من الرابط...")
    ...
    try:
        ...
        if result["type"] == "video":
            await bot.send_video(message.chat.id, result["path"], caption=caption, supports_streaming=True)
        else:
            await bot.send_document(message.chat.id, result["path"], caption=caption)
        await status.delete()
